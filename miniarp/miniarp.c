...
snd_seq_t *open_seq() {
  ...
  if ((port_out_id = snd_seq_create_simple_port(seq_handle, "miniArp_out",
            SND_SEQ_PORT_CAP_READ|SND_SEQ_PORT_CAP_SUBS_READ,
            SND_SEQ_PORT_TYPE_APPLICATION)) < 0) {
    fprintf(stderr, "Error creating sequencer port.\n");
  }
  if ((port_in_id = snd_seq_create_simple_port(seq_handle, "miniArp_in",
...
void parse_sequence() {
  ...
  while(c!='\n') {
    ...
    c =  fgetc(f);
    sequence[1][seq_len] = int(TICKS_PER_QUARTER / atoi(&c));
    c =  fgetc(f);
    sequence[0][seq_len] = int(TICKS_PER_QUARTER / atoi(&c));
    seq_len++;
